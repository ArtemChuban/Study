...
def _psi(n,a,b,q,pr):
    ...
    if n%2==0:
        ...
        if n==4:
            if pr==0:
                ...
                return 4*2*(x^3+a*x+b)*(x^6+5*a*x^4+20*b*x^3-5*a^2*x^2-4*a*b*x-8*b^2-a^3)
    ...
    if n%2!=0:
        pr=0
        return _psi(((n-1)/2+2),a,b,q,pr)*(_psi(((n-1)/2),a,b,q,pr))^3-_psi(((n-1)/2-1),a,b,q,pr)*(_psi(((n-1)/2+1),a,b,q,pr))^3
    if n%2==0:
        if pr==0:
            pr=1
            return (1/(2*y))*_psi(n/2,a,b,q,pr)*(_psi((n/2+2),a,b,q,pr)*(_psi((n/2-1),a,b,q,pr))^2-_psi((n/2-2),a,b,q,pr)*(_psi((n/2+1),a,b,q,pr))^2)
        else:
            return (1/(2*2*(x^3+a*x+b)))*_psi(n/2,a,b,q,pr)*(_psi((n/2+2),a,b,q,pr)*(_psi((n/2-1),a,b,q,pr))^2-_psi((n/2-2),a,b,q,pr)*(_psi((n/2+1),a,b,q,pr))^2)
